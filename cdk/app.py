        ...
        ec2 = _ec2.Instance(
            ...
            init=_ec2.CloudFormationInit.from_elements(
                ...
                _ec2.InitCommand.shell_command(
                     'cd /app && unzip db.zip -d setup && docker build -t db-instance . && docker run --name db-openaq --env-file env --publish 5432:5432 -idt db-instance'
                ),
            ),
            vpc=vpc,
            security_group=sg,
...
Tags.of(cac).add("Project", "CAC")
Tags.of(staging).add("Project", "OpenAQ")
